...
mod connection {
    ...
    impl ClientConnection {
        ...
        /// Make a new ClientConnection with a ShadowTLS-style session ID generator.
        pub fn new_with_session_id_generator(
            config: Arc<ClientConfig>,
            name: ServerName<'static>,
            session_id_generator: impl Fn(&[u8]) -> [u8; 32] + Send + Sync + 'static,
        ) -> Result<Self, Error> {
            Self::new_with_alpn_and_session_id_generator(
                config.clone(),
                name,
        ...
        /// Make a new ClientConnection with custom ALPN and a session ID generator.
        pub fn new_with_alpn_and_session_id_generator(
            config: Arc<ClientConfig>,
            name: ServerName<'static>,
            alpn_protocols: Vec<Vec<u8>>,
            session_id_generator: impl Fn(&[u8]) -> [u8; 32] + Send + Sync + 'static,
...
impl ConnectionCore<ClientConnectionData> {
    pub(crate) fn for_client(
        ...
        Self::for_client_with_session_id_generator(config, name, extra_exts, proto, None)
    }
    ...
    pub(crate) fn for_client_with_session_id_generator(
        ...
        extra_exts: ClientExtensionsInput<'static>,
        proto: Protocol,
        session_id_generator: Option<hs::SessionIdGenerator>,
    ) -> Result<Self, Error> {
        ...
        let input = ClientHelloInput::new(name, &extra_exts, &mut cx, config, session_id_generator)?;
